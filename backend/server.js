...
const MONGO_URI = process.env.MONGO_URI || "mongodb://localhost:27017/gmart";
...
async function getPayPalAccessToken(){
  if (!PAYPAL_CLIENT || !PAYPAL_SECRET) throw new Error('PayPal credentials not set in env');
...
app.get('/api/products', async (req, res) => {
  const p = await Product.find().sort({ name: 1 });
  res.json(p);
});
...
app.post('/api/create-paypal-order', async (req, res) => {
  try {
    ...
    if (!orderRes.ok) {
      ...
      throw new Error(`PayPal order creation failed: ${orderRes.status} ${txt}`);
    }
...
app.post('/api/capture-paypal-order', async (req, res) => {
  try {
    ...
    if (!capRes.ok) {
      ...
      throw new Error(`PayPal capture failed: ${capRes.status} ${txt}`);
    }
    ...
    if (order) {
      ...
      try {
        await transporter.sendMail({
          ...
          subject: 'G Mart - Your Invoice',
          text: 'Thank you for your order. Invoice attached.',
          attachments: [{ filename: `${order._id}.pdf`, path: pdfPath }]
        });
      } catch (e) {
